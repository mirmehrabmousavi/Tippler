                    ...
                    <div class="card-content">
                        <div class="table-responsive mt-1">
                            ...
                            <table class="table table-striped mb-0">
                                ...
                                <tbody>
                                @foreach($mags as $mag)
                                <tr id="dataRow-{{$mag->id}}">
                                    <td>
                                        <fieldset>
                                            <div class="custom-control custom-checkbox">
                                                <input type="checkbox" class="custom-control-input sub_chk" name="checkbox[]" data-id="{{$mag->id}}" id="check-{{$mag->id}}">
                                                <label class="custom-control-label" for="check-{{$mag->id}}"></label>
                                            </div>
                                        </fieldset>
                                    </td>
                                    <td>{{$loop->index+1}}</td>
                                    <td><a href="#">{{$mag->title}}</a></td>
                                    <td class="p-1">
                                    ...
                                    <td>
                                        ...
                                        @else
                                        <a class="btn btn-sm btn-outline-secondary text-white" data-toggle="modal" data-target="#confirm-{{$mag->id}}"><i class="feather icon-x"></i> انتشار محتوا </a>
                                        <div class="modal fade text-left" id="confirm-{{$mag->id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel120" aria-hidden="true" style="display: none;">
                                            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header bg-outline-secondary white">
                                                        <h5 class="modal-title" id="myModalLabel120">از انتشار محتوا با نام "{{$mag->title}}" اطمینان داری؟</h5>
                                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
                                                    </div>
                                                    <div class="modal-body"></div>
                                                    <div class="modal-footer">
                                                        <form action="{{route('admin.mags.confirm', [$mag->id])}}"
                                                              method="POST">
                                                            @csrf
                                    ...
                                    <td><span class="badge badge-light rounded-pill">{{$mag->views}}</span></td>
                                    <td>
                                        <a href="{{route('admin.mags.edit', ['mag' => $mag->id])}}" class="text-secondary mr-1 waves-effect waves-light"><i class="feather icon-edit"></i></a>
                                        <a class="text-secondary mr-1 waves-effect waves-light text-danger" data-toggle="modal" data-target="#delete-{{$mag->id}}"
                                           data-tr="tr_{{$mag->id}}"
                                        ...
                                        <div class="modal fade text-left" id="delete-{{$mag->id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel120" aria-hidden="true" style="display: none;">
                                            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header bg-danger white">
                                                        <h5 class="modal-title" id="myModalLabel120">از حذف محتوا با نام "{{$mag->title}}" اطمینان داری؟</h5>
                                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
                                                    </div>
                                                    <div class="modal-body">
                                                    </div>
                                                    <div class="modal-footer">
                                                        <form action="{{route('admin.mags.destroy', [$mag->id])}}"
                                                              method="POST">
                                                            @csrf
                                                            @method('DELETE')
                                ...
                                <script>
                                    $('#check-{{$mag->id}}').on('change', function() {
                                        var isChecked = $(this).prop('checked');
                                        var $tr = $('#dataRow-{{$mag->id}}');
                            ...
                            {{$mags->links('admin.pagination.paginate')}}
                            @else
                                <div class="alert alert-danger mx-1 text-center"><i class="fa fa-info-circle"></i> مجله ای(مقاله) وجود ندارد</div>
                            @endif
                        </div>
                    </div>
                </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.delete_all').on('click', function(e) {
                ...
                if(allVals.length <=0)
                {
                    alert("لطفا یک مقاله انتخاب کنید");
                }  else {

                    var check = confirm("مطمئنی می خوای این مقالات رو پاک کنی؟");
                    if(check === true){
                        var join_selected_values = allVals.join(",");
                        $.ajax({
                            url: '{{route('admin.mags.destroyAll')}}',
                            type: 'DELETE',
